...
async function ensureNoExistingWindow() {
  const existingWindowId = await findWindowId(WINDOW_TITLE);
  if (existingWindowId) {
    console.log(`Closing existing window ID: ${existingWindowId}`);
...
async function createNewWindow() {
  ...
  await $`sleep 0.1`.quiet(); // Minimal wait for window creation
}

async function setupWindowLayout() {
...
async function openWorkoutNote() {
  try {
    ...
    await ensureNoExistingWindow();
    await createNewWindow();
    await setupWindowLayout();
